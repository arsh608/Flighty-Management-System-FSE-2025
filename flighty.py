...
def bookticket():
    if request.method == 'GET':
        ...
        if request.form['submit_button'] == 'select_srcdest':
            ...
            # Fetch location codes for selected cities
            cursor.execute(
                "SELECT a1.location_code, a2.location_code FROM airport a1, airport a2 WHERE a1.city = %s AND a2.city = %s",
                (_src, _dst)
            )
            airports = cursor.fetchone()

            if not airports:
                flash('Invalid source or destination.')
                return render_template('app.html', src_dest=(_src, _dst), error='Invalid airport selection.')

            src_code, dst_code = airports

            # Debug print (optional)
            print("User selected:", _src, "to", _dst)
            print("Resolved codes:", src_code, dst_code)

            # Updated query: robust matching using TRIM + LOWER
            cursor.execute("""
                SELECT flight.*, airline.airline_name
                FROM flight
                JOIN flightScheduledForAirline fsa ON flight.flight_id = fsa.flight_id
                JOIN airline ON fsa.airline_id = airline.airline_id
                JOIN airport src_airport ON LOWER(TRIM(flight.source)) = LOWER(TRIM(src_airport.city))
                JOIN airport dst_airport ON LOWER(TRIM(flight.destination)) = LOWER(TRIM(dst_airport.city))
                WHERE src_airport.location_code = %s
                  AND dst_airport.location_code = %s
                  AND flight.flight_id NOT IN (
                      SELECT flight_id FROM ticket WHERE uname = %s
                  )
            """, (src_code, dst_code, session['uname']))
            ...
            if flights:
                ...
                return render_template('app.html', src_dest=(_src, _dst), error='No flights available.')
        else:
            # Submit button contains flight number
